        ...
        {!quizFinished ? (
          <AnimatePresence mode="wait">
            ...
            >
                    ...
                    {Array.from({ length: questions.length }).map((_, i) => (
                      <div
                        ...
                        className={`w-2 h-2 rounded-full transition-colors ${
                          i < currentQuestion ? (questions[i] && selectedAnswer === questions[i].correctIndex ? 'bg-green-500' : 'bg-stone-200')
              ...
              {/* Question */}
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 border border-purple-200/30 shadow-sm mb-4">
                <div className="flex items-start gap-3 mb-4">
                  <div className="w-10 h-10 rounded-full bg-purple-100 flex items-center justify-center shrink-0">
                    <span className="text-lg font-bold text-purple-700">{currentQuestion + 1}</span>
                  </div>
                  <h2 className="text-base font-semibold text-stone-800 leading-relaxed pt-1.5">
                ...
                <div className="space-y-2 mt-4">
                  {questions[currentQuestion]?.options.map((option, idx) => {
                    ...
                    let optionStyle = 'border-stone-200/50 bg-stone-50/50 hover:bg-amber-50 hover:border-amber-200';

                    if (showResult) {
                      if (isCorrect) optionStyle = 'border-green-300 bg-green-50';
                      else if (isSelected && !isCorrect) optionStyle = 'border-red-300 bg-red-50';
                      else optionStyle = 'border-stone-200/30 opacity-50';
                    } else if (isSelected) {
                      optionStyle = 'border-purple-300 bg-purple-50';
                    }
          ...
          >
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-8 border border-amber-200/30 shadow-sm mb-4">
              <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
